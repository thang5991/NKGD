...
export function useBlog() {
  ...
  const savePostWithImages = useCallback(
    async (formData: BlogPostFormData): Promise<BlogPost> => {
  ...
  const loadPostImages = useCallback(async (imageRefs: string[]): Promise<ImageRecord[]> => {
    if (!imageRefs || imageRefs.length === 0) return [];
    return getImagesByIds(imageRefs);
  }, []);

  const getPost = useCallback(async (id: string): Promise<BlogPost | undefined> => {
